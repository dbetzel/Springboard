...
the member name. */
SELECT name, CONCAT(M.surname,  ',',  M.firstname) AS member, 
FROM (
...
SELECT F.name, CONCAT(M.surname,  ',',  M.firstname) AS member, 
...
SELECT name, CONCAT(surname, ',', firstname) AS member, COUNT(surname) AS 'usage' 
FROM Bookings B 
LEFT JOIN Facilities F ON B.facid = F.facid 
